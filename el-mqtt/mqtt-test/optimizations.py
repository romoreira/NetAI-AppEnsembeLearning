...
# =========================
def evaluate_weighted_probs(probs_list, weights, true_labels, args, method, round_id=None):
    """
    Combina vetores de probabilidade usando pesos e calcula a acurácia final.
    Também salva o classification_report por método e round para GA/PSO.
    """
    combined = np.zeros_like(probs_list[0])
    for i, probs in enumerate(probs_list):
    ...
    # Salvar classification report com método e round (evita sobrescrever)
    ...
    if round_id is not None:
        suffix += f"_round{round_id}"

    report_path = os.path.join(out_dir, f"classification_report{suffix}.txt")
    with open(report_path, "w") as f:
        f.write(f"Classification Report{(' - ' + method) if method else ''}")
        if round_id is not None:
