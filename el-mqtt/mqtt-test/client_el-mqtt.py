...
MQTT_TOPIC_OUT = f"{client_str}/probs"
MQTT_TOPIC_IN  = f"server/teacher/{args.combo_name}/{args.ensemble_method}"
...
def _save_classification_artifacts(probs: torch.Tensor,
    ...
    """
    # Dir do round
    round_dir = os.path.join(CLIENT_RESULTS_PATH, f"round_{round_id}")
...
def publish_probs(mqttc, round_id, probs, labels):
    ...
    info = mqttc.publish(MQTT_TOPIC_OUT, json.dumps(payload), qos=2)
...
while current_round <= total_rounds:
    ...
    for epoch in range(args.kd_epochs):
        ...
        for inputs, y_local in pbar:
            ...
            # NEW: consistência com as probs do round-1 (evita colapso/diversidade cai)
    ...
    # ADIÇÃO: após treinar com KD no round atual, roda avaliação e salva AFTER
    ...
    if current_round <= total_rounds:
        probs, labels = extract_probs_current_model()
        print(f"[EVAL] Round {current_round} - amostras: {probs.shape[0]}, classes: {probs.shape[1]}")
        # ADIÇÃO opcional: salvar o BEFORE do próximo round (estado atual antes de publicar de novo)
        _save_classification_artifacts(probs, labels, classes, round_id=current_round, phase="before")
